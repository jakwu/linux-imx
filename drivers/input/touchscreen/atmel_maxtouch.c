...
#include <linux/input/atmel_maxtouch.h>
...
#ifdef MXT_AUTOCAL
static int recalib;
#define MXT_AUTOCAL_CYCLE_MS    1100     /* 1.1s */
#endif /* MXT_AUTOCAL */

#define DRIVER_VERSION "0.92d"

#ifdef MXT_DEBUG
static int debug = 1;   //DEBUG_TRACE;
#else
...
#ifdef MXT_AUTOCAL
	struct delayed_work  auto_cal_work;
#endif /* MXT_AUTOCAL */
...
static u8 mxt_read_touch_gpio(struct mxt_data *mxt)
{
	return gpio_get_value(mxt->touch_gpio);
}

static u8 mxt_is_valid_interrupt(struct mxt_data *mxt)
{
	return !mxt_read_touch_gpio(mxt);
}
...
static void repeatTimer_cb(unsigned long data)
{
	...
	input_report_abs(gs_repeatData.input, ABS_PRESSURE, MXT_MAX_REPORTED_PRESSURE);
...
static const u8	*_object_type_name[] = {
    [0]  = "Reserved",
...
static void object_type_name_get(uint8_t object_type, u8 *object_type_name, int object_type_name_size)
{
        if (object_type < ARRAY_SIZE(_object_type_name)){
                const u8 *obj_name = _object_type_name[object_type];
                if (obj_name != NULL) {
                        strncpy(object_type_name, obj_name, object_type_name_size);
                        return;
                }
        }

        snprintf(object_type_name, object_type_name_size, "UNKNOWN_T%02d", object_type);
}
			...
			loff_t *ppos, u8 debug_command)
{
	...
	if (mxt->current_debug_datap == 0) {
		...
		while (size > 0) {
			...
			//printk("read out debug data page = %d byte %d\n", page,read_size);
			mxt_debug(DEBUG_TRACE,
...
int mxt_message_release(struct inode *inode, struct file *file)
{
	struct mxt_data *mxt;
...
static int program_object_data(struct i2c_client *client, struct mxt_data *mxt)
{
	...
	while (data_read != size) {
		...
		for (i=0;i<count;i++) {
			printk(KERN_INFO "0x%02X ", object_data[data_read]);
...
static long mxt_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	int retval =0,i=0,j=0;
	u8 value=0;
	struct mxt_data *mxt;
	mxt = filp->private_data;
	...
	case MXT_PRINT_OBJECTS:
		for (i = 0;i<mxt->device_info.num_objs;i++) {
                        u8 object_type_name[32];
                        object_type_name_get(mxt->object_table[i].type, object_type_name, sizeof(object_type_name));
			for (j=0;j<mxt->object_table[i].size;j++){
				mxt_read_block(mxt->client,MXT_BASE_ADDR(mxt->object_table[i].type, mxt) + j,1,&value);
				printk(KERN_INFO "MXT_PRINT_OBJECTS %s.%02d 0x%02X\n",object_type_name, j, value);
			}
		}
		break;
	default:
...
void process_T9_message(u8 *message, struct mxt_data *mxt, int last_touch)
{
	...
	if (last_touch) {
		...
		if (stored_size[0]) {
			...
			input_report_abs(mxt->input, ABS_PRESSURE, MXT_MAX_REPORTED_PRESSURE);
			input_report_abs(mxt->input, ABS_X, stored_x[0]);
			input_report_abs(mxt->input, ABS_Y, stored_y[0]);
#ifdef MXT_DEBUG
			printk("BTN_TOUCH=1, ABS_PRESSURE=%3d, ABS_X=%5d, ABS_Y=%5d\n", MXT_MAX_REPORTED_PRESSURE, stored_x[0], stored_y[0]);
#endif
...
#ifdef MXT_DEBUG
			printk("BTN_TOUCH=0, ABS_PRESSURE=0\n");
#endif
			input_sync(mxt->input);
			repeatStop();
		}
...
int process_message(u8 *message, u8 object, struct mxt_data *mxt)
{
	...
	case MXT_PROCG_NOISESUPPRESSION_T48:
		if (debug >= DEBUG_TRACE)
			dev_info(&client->dev, "Receiving PROCG_NOISESUPPRESSION_T48 message...\n");

		status = message[MXT_MSG_T48_STATUS];

		if (status & MXT_MSGB_T48_ALGOERR) {
			int retval;

			dev_err(&client->dev, "maXTouch: T48_ALGOERR\n");

			/* chip is blocked on ALGOERR, reset neccessary (SROS, 2013-03-18) */
			retval = mxt_write_byte(mxt->client, MXT_BASE_ADDR(MXT_GEN_COMMANDPROCESSOR_T6, mxt) + MXT_ADR_T6_RESET, 1);
			if (retval < 0)
			  dev_err(&client->dev, "maXTouch: mxt_write_byte(MXT_ADR_T6_RESET): retval=%d\n", retval);
		}
		break;
...
//#ifdef MXT_AUTOCAL
//	cancel_delayed_work(&mxt->auto_cal_work);
//#endif /* MXT_AUTOCAL */
	...
	do {
		...
		if (0) {
			...
			object_type_name_get(report_id, object_type_name, sizeof(object_type_name));
		...
		mxt_debug(DEBUG_TRACE, "mxt_read_touch_gpio(): %d\n", mxt_read_touch_gpio(mxt));
	} while (comms ? (mxt_read_touch_gpio(mxt) == 0) :
...
//#ifdef MXT_AUTOCAL
//	schedule_delayed_work(&mxt->auto_cal_work, msecs_to_jiffies(MXT_AUTOCAL_CYCLE_MS));
//#endif /* MXT_AUTOCAL */
...
static irqreturn_t mxt_irq_handler(int irq, void *_mxt)
{
	...
	if (!mxt_is_valid_interrupt(mxt)) {
		printk(KERN_WARNING "%s %s %d: spurious interrupt\n", __FILE__, __FUNCTION__, __LINE__);
...
#ifdef MXT_AUTOCAL
static void auto_cal_worker(struct work_struct *work)
{
	struct mxt_data *mxt;
	int retval = 0;
	int schedule_factor = 1;
	mxt = container_of(work, struct mxt_data, auto_cal_work.work);
	if (recalib == 1) {
		retval = mxt_write_byte(mxt->client,
			      MXT_BASE_ADDR(MXT_GEN_COMMANDPROCESSOR_T6, mxt) +
			      MXT_ADR_T6_CALIBRATE,
	...
	if (retval < 0)
		printk("%s %s %d: retval=%d\n", __FILE__, __FUNCTION__, __LINE__, retval);

	cancel_delayed_work(&mxt->auto_cal_work);
	schedule_delayed_work(&mxt->auto_cal_work, msecs_to_jiffies(schedule_factor*MXT_AUTOCAL_CYCLE_MS));
}
#endif /* MXT_AUTOCAL */
				  ...
				  u8 *id_block_data)
{
	...
	/* MXT224S */
	} else if (mxt->device_info.family_id == MXT224S_FAMILYID) {
		strcpy(mxt->device_info.family_name, "mXT224S");

		if (mxt->device_info.variant_id == MXT224S_CAL_VARIANTID) {
			strcpy(mxt->device_info.variant_name, "Calibrated");
		} else {
			dev_err(&client->dev,
				"Warning: maXTouch Variant ID [%d] not "
					   ...
					   u8 *raw_id_data)
{
	...
	for (i = 0; i < mxt->device_info.num_objs; i++) {
		...
		/* Read out extented object table in T254 object */
		// There are a few things not defined in the protocol (e.g. ext_ojbect_table size) to complete the implementation of the ext_object_table
		/*if (object_type == 254) {
			for (j=0;j<count_ext_ojbects;j++) {
				mxt_debug(DEBUG_TRACE, "Reading maXTouch at [0x%04x]: ",
				  object_info_address);


				error = mxt_read_block(client, object_address+(j*MXT_EXT_OBJECT_TABLE_ELEMENT_SIZE),
						       MXT_EXT_OBJECT_TABLE_ELEMENT_SIZE, buf);
				...
				memcpy(raw_ib_data + ib_pointer, buf,
				       MXT_EXT_OBJECT_TABLE_ELEMENT_SIZE);
				ib_pointer += MXT_OBJECT_TABLE_ELEMENT_SIZE;
...
static struct mxt_platform_data *mxt_parse_dt(struct device *dev)
{
	...
	if (!pd) {
		dev_err(dev, "error, failed to allocate platform data\n");
		return NULL;
	}

	if (of_property_read_u32(np, "numtouch", &val)) {
		dev_err(dev, "error, missing mandatory 'numtouch' property\n");
		goto err_property_read;
	}

	if (val > 10)  {
		dev_err(dev, "error, 'numtouch' property out of range (0 < numtouch <= 10)\n");
		goto err_property_read;
	}

	pd->numtouch = val;

	if (of_property_read_u32(np, "max-x", &pd->max_x)) {
		dev_err(dev, "error, missing mandatory 'max-x' property\n");
		goto err_property_read;
	}

	if (of_property_read_u32(np, "max-y", &pd->max_y)) {
		dev_err(dev, "failed to get mandatory 'max-y' property\n");
		goto err_property_read;
	}

	pd->touch_gpio = of_get_named_gpio(np, "touch-gpio",  0);
	if (pd->touch_gpio < 0) {
		dev_err(dev, "error, missing mandatory 'touch-gpio' property\n");
		goto err_property_read;
	}

	ret = gpio_request_one(pd->touch_gpio, GPIOF_DIR_IN, "touch-gpio");
	if (ret) {
		dev_err(dev, "error, failed to request 'touch-gpio'\n");
			       ...
			       const struct i2c_device_id *id)
{
	...
	mxt->touch_gpio        = pdata->touch_gpio;

	mxt_debug(DEBUG_INFO, "max_x      = %d\n", mxt->max_x_val);
	mxt_debug(DEBUG_INFO, "max_y      = %d\n", mxt->max_y_val);
	mxt_debug(DEBUG_INFO, "numtouch   = %d\n", mxt->numtouch);
	mxt_debug(DEBUG_INFO, "touch_gpio = %d\n", mxt->touch_gpio);
...
#ifdef MXT_AUTOCAL
	INIT_DELAYED_WORK(&mxt->auto_cal_work, auto_cal_worker);
#endif /* MXT_AUTOCAL */
...
static int mxt_resume(struct i2c_client *client)
{
	...
	disable_irq_nosync(mxt->irq); /* schedule_delayed_work below calls mxt_worker, mxt_worker calls enable_irq(mxt->irq) */
	cancel_delayed_work(&mxt->dwork);
	schedule_delayed_work(&mxt->dwork, 0);
	return 0;
}
